...
class DeviceProtocol:
    ...
    @staticmethod
    def teacher_power_cmd(val: bool, voltage_val: int = 1200, current_val: int = 2500, is_ac: bool = False) -> bytes:
        """
        Device B (Port 8888) Teacher Power Control
        Protocol: Modbus RTU over TCP (No Header)
        Matches logic from archived_v1 (10.168.1.211:8887):
        - Reg 3: Current (Default 2500 = 2.5A)
        - Reg 4: Voltage (x100)
        - Reg 5: Enable (1=ON)
        
        Updated: Slave IDs match Student Groups (A1/A2)
        """
        # User Correction: Slave 0xA1 (DC), 0xA2 (AC) - same as Group 1
        slave = 0xA2 if is_ac else 0xA1
        
        if val:
            # Turn ON (Set Params & Enable)
            # Reg 3 = Current (0.001A steps? 2.5A=2500), Reg 4 = Voltage (0.01V steps? 8.5V=850)
            # Frame: Slave 10 00 03 00 04 08 [II II] [VV VV] 00 01 00 00
            ...
            # PDU Head: Slave, Func(10), Addr(0003), Qty(0004), Bytes(08)
            pdu_head = struct.pack('>BBHHB', slave, 0x10, 0x0003, 0x0004, 0x08)
            pdu = pdu_head + data_payload
            return pdu + crc16_modbus(pdu)
        else:
            # Turn OFF
            # Doc: Write Addr 0x0004, Len 2, Zeros
